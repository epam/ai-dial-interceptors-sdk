...
_PRESIDIO_NLP_MODEL_TRAINING_DATA = get_env_optional(
...
def create_custom_trained_model(default_nlp_model: Language, lang: str):
    ...
    load_custom_trained_model(
        trained_model, get_custom_trained_model_name(lang)
    )
    _log.info(
        f"Custom trained model based on {str(default_nlp_model)} has been created"
    )
...
def get_training_data() -> List[dict]:
    ...
    try:
        ...
        if not all(isinstance(item, tuple) for item in training_data):
            raise TypeError(
                "Each item in 'PRESIDIO_NLP_MODEL_TRAINING_DATA' must be a tuple."
            )

        if not all(len(item) == 2 for item in training_data):
            raise TypeError(
                "Each tuple in 'PRESIDIO_NLP_MODEL_TRAINING_DATA' must have exactly 2 elements."
            )
        ...
        if not all(
            ...
            raise TypeError(
                "Each tuple in 'PRESIDIO_NLP_MODEL_TRAINING_DATA' must be a (string, dictionary)."
            )

        return training_data
    except (SyntaxError, ValueError) as e:
        raise ValueError(
            f"Training data is not valid Python literal format: {e}"
        )
