...
2. **Create** a new project & attach it to a **Billing Account**.
3. **Enable** the Model Armor, Compute, and KMS APIs.
4. **Create** four templates (basic filter, inspect, de‑id, re‑id).
5. **Provision** a Cloud KMS key ring + key, generate an AES‑256 DEK, and **wrap** it.
6. **Export** the wrapped key to `.env` (or Secret Manager).
...
See variables list by the link [environment variable](#environment-variables)
---

## 2  Authenticate

```bash
# 2.1  Activate the service account
gcloud auth activate-service-account        --key-file ./secret/xxx‑key.json

# 2.2  (Optionally) set it as the active account
gcloud config set account "$(jq -r .client_email < ./secret/xxx‑key.json)"
```

---

## 3  Create & Fund the Project

```bash
# 3.1  Create the project
gcloud projects create "$PROJECT_ID"        --name "Model Armor Demo"

# 3.2  Link to a billing account
...
### 5.3  De‑identify Template (strip findings)
...
### 5.4  Re‑identify Template (surrogate tokens)
...
## 7  Generate & Wrap a Data‑Encryption Key (DEK)

```bash
# 7.1  Generate 256‑bit AES key
openssl rand -out ./aes_key.bin 32

# 7.2  Base‑64 encode
PLAINTEXT_KEY=$(base64 -i ./aes_key.bin)

# 7.3  Wrap using Cloud KMS
